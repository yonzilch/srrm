...
authRoutes.get("/login", async (c) => {
  ...
  try {
    ...
    return c.redirect(
      `${oidcConfig.authorization_endpoint}?${urlParams.toString()}`,
    );
  } catch (err: unknown) {
    console.error(
      "[OIDC Config Error]",
...
authRoutes.get("/callback", async (c) => {
  ...
  try {
    ...
    const adminEmails = ADMIN_EMAILS.split(",").map((e: string) =>
      e.trim(),
    );
    const role: "admin" | "viewer" = adminEmails.includes(email)
      ? "admin"
      : "viewer";
