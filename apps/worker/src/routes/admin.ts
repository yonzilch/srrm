...
import {
  ...
  deleteRepo,
  getLatestReleases,
  deleteReleasesForRepo,
  getRepoReleaseCounts,
} from "../services/db";
  ...
  try {
    ...
    if (!detected) {
      return c.json(
        {
          error:
            "无法识别仓库地址，请检查格式。支持：完整 URL 或 owner/repo",
        },
        400,
      );
    }
    ...
    return c.json(
      {
        ok: false,
        message:
          "未配置任何通知方式（GOTIFY_URL / APPRISE_API_URL / WEBHOOK_URL 均未设置）",
      },
      400,
    );
  }
...
adminRoutes.get("/notify/status", async (c) => {
  const { GotifyNotifier, AppriseNotifier, WebhookNotifier } =
    await import("../services/notifiers/index");
  const all = [
    new GotifyNotifier(),
    new AppriseNotifier(),
