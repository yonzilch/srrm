...
export async function addRepo(
  ...
  const existing = await getRepoByFullNameAndBase(
    db,
    repo.fullName,
    repo.baseUrl,
  );
...
export async function upsertReleases(
  ...
  const existRows = await db
    .prepare(
      `SELECT repo_full_name, tag_name FROM releases WHERE (repo_full_name, tag_name) IN (VALUES ${placeholders})`,
    )
    .bind(...existParams)
    .all();

  const existingSet = new Set(
    (existRows.results as any[]).map(
      (row: any) => `${row.repo_full_name}|${row.tag_name}`,
    ),
  );
  ...
  return {
    inserted: newReleases.length,
    updated: updateReleases.length,
    newReleases,
  };
}
...
export async function getReleaseDates(
  ...
  const { results } = await db
    .prepare(
      "SELECT DISTINCT date(published_at) as d FROM releases ORDER BY d DESC LIMIT ?",
    )
    .bind(lim)
    .all();
...
export async function deleteReleasesForRepo(
  ...
  await db
    .prepare("DELETE FROM releases WHERE repo_full_name = ?")
    .bind(repoFullName)
    .run();
}
...
export async function getRepoReleaseCounts(
  db: D1Database,
): Promise<Record<string, number>> {
  const { results } = await db
    .prepare(
      "SELECT repo_full_name, COUNT(*) as cnt FROM releases GROUP BY repo_full_name",
    )
    .all();
...
export async function getLatestReleases(
  ...
  const { results } = await db
    .prepare(
      "SELECT * FROM releases ORDER BY published_at DESC LIMIT ?",
    )
    .bind(lim)
    .all();
...
export const getReleasesByDate = (
  db: D1Database,
  date: string,
): Promise<Release[]> =>
  getReleases(db, { date }).then((r) => r.releases);

export const getReleasesIndex = (db: D1Database): Promise<string[]> =>
  getReleaseDates(db);

export const saveReleasesByDate = (
  db: D1Database,
  _date: string,
  releases: Release[],
): Promise<void> => upsertReleases(db, releases).then(() => undefined);

export const updateLatestReleases = (
  db: D1Database,
  newReleases: Release[],
): Promise<void> =>
  upsertReleases(db, newReleases).then(() => undefined);

export async function updateReleasesIndex(
  _db: D1Database,
  _date: string,
): Promise<void> {
