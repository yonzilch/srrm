...
import { getRepos, saveReleases, getLastRun, saveLastRun } from './kv';
...
export async function runScraper(env: Env): Promise<void> {
  ...
  const storedIds = new Set(storedReleases.map(r => r.id));
  
  // 只保存新的 releases
  const newReleases = allReleases.filter(release => !storedIds.has(release.id));
